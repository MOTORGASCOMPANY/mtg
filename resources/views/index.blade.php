<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <form action="{{ $comunicado ? route('comunicado.update') : route('comunicado.store') }}" method="POST"
                    ...
                    <div class="mb-4">
                        ...
                        <x-jet-input type="text" name="titulo" id="titulo"
                            value="{{ $comunicado['titulo'] ?? old('titulo') }}" class=" w-full" />
                        @error('titulo')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="contenido" id="contenido" rows="5"
                            class="shadow appearance-none border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            style="height: 300px;">{{ $comunicado['contenido'] ?? old('contenido') }}</textarea>
                        @error('contenido')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div id="imageAlbum" class="flex flex-wrap mt-4 hidden">
                            <label class="mr-4 mb-4 cursor-pointer">
                                <input type="radio" name="imagen" value=""
                                    {{ !isset($comunicado['imagen']) ? 'checked' : '' }} class="hidden">
                                <span
                                    class="w-24 h-24 flex items-center justify-center border-2 border-gray-300 rounded text-gray-500 hover:border-indigo-500">
                                    Ninguna
                                </span>
                            </label>
                                ...
                                @if (in_array(strtolower($file->getExtension()), ['png', 'jpg', 'jpeg']))
                                    <label class="mr-4 mb-4 cursor-pointer">
                                        <input type="radio" name="imagen"
                                            value="{{ 'images/images/' . basename($file) }}"
                                            {{ isset($comunicado['imagen']) && $comunicado['imagen'] == 'images/images/' . basename($file) ? 'checked' : '' }}
                                            class="hidden">
                                        <img src="{{ asset('images/images/' . basename($file)) }}"
                                            alt="{{ basename($file) }}"
                                            class="w-24 h-24 object-cover border-2 border-gray-300 rounded hover:border-indigo-500 {{ isset($comunicado['imagen']) && $comunicado['imagen'] == 'images/images/' . basename($file) ? 'border-indigo-500' : '' }}">
                                    </label>
                                @endif
                            @endforeach
                        </div>
                    ...
                    <div class="flex items-center justify-center">
                        ...
                        @if ($comunicado)
                            <x-jet-secondary-button onclick="deactivateComunicado()"
                                class="text-indigo-500 border-indigo-400">
                                Inhabilitar
    ...
    <script>
        ...
        function deactivateComunicado() {
            fetch('{{ route('comunicado.deactivate') }}', {
                method: 'POST',
                headers: {
