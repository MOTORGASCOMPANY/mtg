<div class="bg-gray-200 bg-opacity-25 flex flex-col items-center justify-center px-4">
        ...
        @if (isset($servicios))
                ...
                <div class="grid lg:gap-x-12 lg:grid-cols-3">
                    <div class="mb-12 lg:mb-0">
                        <div class="rounded-lg shadow-lg h-full block bg-white">
                            ...
                            <div class="p-6">
                                <h3 class="text-2xl font-bold text-indigo-600 mb-4">{{ $cantidad }}</h3>
                    ...
                    <div class="mb-12 lg:mb-0">
                        <div class="rounded-lg shadow-lg h-full block bg-white">
                            ...
                            <div class="p-6">
                                <h3 class="text-2xl font-bold text-indigo-600 mb-4"><span>S/.</span>
                                    {{ $monto }}</h3>
            ...
            <div class="w-full">
                ...
                <div style="position: relative; height:30vh; width:100%">
                    <canvas id="graficoServicios" class="w-full m-auto" wire:ignore>
                    </canvas>
                </div>
                <div class="mb-4">
                    @switch($periodo)
                        @case(1)
                            <p class="flex text-center w-full justify-center">
                                <span class="text-white bg-indigo-300 p-1 rounded-md">DEL
                                    {{ date('d-m-Y', strtotime($fecha['fechaInicio'])) }}</span>
                            </p>
                        @break
                        ...
                        @case(2)
                            <p class="flex text-center w-full justify-center">
                                <span class="text-white bg-indigo-300 p-1 rounded-md">DEL
                                    {{ date('d-m-Y', strtotime($fecha['fechaInicio'])) . ' AL ' . date('d-m-Y', strtotime($fecha['fechaFin'])) }}</span>
                            </p>
                        @break
    ...
    <div class="hidden w-full h-screen flex flex-col justify-center items-center "
    ...
    @push('js')
        <script defer>
            ...
            window.addEventListener("load", (event) => {
                const chart2 = new Chart(document.getElementById('graficoServicios'), cfg);
                Livewire.on('updateChart1', ($dataset) => {
                    //const chart1=document.getElementById(data["nombre"]);
                    chart2.data.datasets = $dataset;
                    chart2.update();
                });
            });
        </script>
    ...
    @push('js')
        <script defer>
            window.addEventListener("load", (event) => {
                const chart2 = new Chart(document.getElementById('graficoServicios'), {
                    type: 'bar',
                    data: {
                        labels: @json($labels),
